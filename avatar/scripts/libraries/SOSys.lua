...
hatModelPart = nil -- Список частей модели что являются частью шляпы
...
function pings.setOutfit(outfit)
    if outfit == nil then outfit = 1 end

    currentOutfit = outfit

    -- Задаём текстуру наряда
    for _, value in ipairs(outfitModelParts) do
        value:setPrimaryTexture("Custom", textures[outfitsList[outfit][2]])
    end

    -- Задаём текстуру и высоту шляпы
    if outfitsList[outfit][6] ~= nil then hatModelPart:setPrimaryTexture("Custom", textures[outfitsList[outfit][6]]) end
    hatModelPart:setPos(0, outfitsList[outfit][4], 0)

    -- Задаём и видимость второго слоя головы
    headSecondLayerModelPart:setScale(outfitsList[outfit][5])
end
